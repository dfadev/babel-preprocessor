...
function handleProcInclude(path, state) {
	if (!state.current || state.current && state.current.satisfied && !state.current.remove) {
		...
		if (len > 1) args = eval((0, _babelGenerator2.default)(path.node.expression.arguments[1]).code);
		if (len > 2) opts = eval('(' + (0, _babelGenerator2.default)(path.node.expression.arguments[2]).code + ')');

		var rslt = (0, _child_process.spawnSync)(cmd, args, opts);
...
function handleStringProcessInclude(path, state) {
	if (!state.current || state.current && state.current.satisfied && !state.current.remove) {
		...
		if (len > 1) args = eval((0, _babelGenerator2.default)(path.node.arguments[1]).code);
		if (len > 2) opts = eval('(' + (0, _babelGenerator2.default)(path.node.arguments[2]).code + ')');
